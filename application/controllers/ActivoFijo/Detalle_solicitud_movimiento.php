...
class Detalle_solicitud_movimiento extends CI_Controller {
  ...
  public function index(){
    ...
    $msg = array('alert' => $this->uri->segment(5),'nombre'=> $nombre->observacion,'id_mov' => $id_mov,'controller'=>'detalle_movimiento');
		$data['body'] = $this->load->view('mensajes', $msg, TRUE) . $this->load->view('ActivoFijo/Detalle_Solicitud_movimiento_view',$msg,TRUE) .
    ...
    */
    ...
    if ($this->input->is_ajax_request()) {
      if (!($this->input->post('busca') == "")) {
          ...
          $pagination = paginacion('index.php/ActivoFijo/Detalle_solicitud_movimiento/index/'.$this->uri->segment(4), $this->Detalle_movimiento_model->totalDetalleMovimientos($this->uri->segment(4)),
                        $num, '5');
      }
    } else {
          $registros = $this->Detalle_movimiento_model->obtenerDetalleMovimientosLimit($num, $this->uri->segment(5),$this->uri->segment(4));
    ...
    */
    ...
    if (!($registros == FALSE)) {
      foreach($registros as $det) {
          ...
          '<a class="icono icon-eliminar" uri="'.base_url('index.php/ActivoFijo/Detalle_solicitud_movimiento/EliminarDato/'.$det->id_movimiento.'/'.$det->id_detalle_movimiento.'/').'"></a>');
      }
    } else {
  ...
  */
  public function RecibirDatos(){
    ...
    $data2=array(
      ...
      if (!($this->input->post('id_detalle_movimiento') == '')){
        ...
        redirect('/ActivoFijo/Detalle_solicitud_movimiento/index/'.$data['id_movimiento'].'/update');
      }
      else{
        ...
        redirect('/ActivoFijo/Detalle_solicitud_movimiento/index/'.$data['id_movimiento'].'/new');
      }
  }
  ...
  public function EliminarDato(){
    ...
    redirect('/ActivoFijo/Detalle_solicitud_movimiento/index/'.$id_mov.'/delete');
  }
