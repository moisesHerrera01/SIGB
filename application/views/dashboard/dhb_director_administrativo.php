<section class="dashboard-panel-izq">
  ...
  <div class="dashboard-box">
    <div class="dashboard-box-title">
      <span>Gasto Bodega</span>
      <span class="icono icon-cancel-circle"></span>
      <span class="icono max-min icon-circle-up"></span>
    </div>
    <div class="dashboard-box-content">
      <div class="chart-content">
        <canvas id="gasto_bodega"></canvas>
      </div>
    </div>
  </div>
</section>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $.ajax({
      type: 'post',
      url: baseurl + "index.php/Dashboard/obtenerGastoTotalBodega",
      data: {},
      success: function(result) {
        var res = JSON.parse(result);
        var myBarChart = new Chart($('#gasto_bodega'), {
          type: 'bar',
          data: {
            labels: ["Mes Pasado", "Mes Actual"],
            datasets: [
              {
                label: "Gastos De Bodega",
                backgroundColor: [
                  'rgba(119, 190, 119, 0.2)',
                  'rgba(119, 190, 119, 0.2)'
                ],
                borderColor: [
                  'rgba(119, 190, 119, 1)',
